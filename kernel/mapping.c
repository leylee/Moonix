...
void getVpnLevels(usize vpn, usize *vpn_levels)
{
    vpn_levels[0] = (vpn >> 18) & 0x1ff;
    vpn_levels[1] = (vpn >> 9) & 0x1ff;
    vpn_levels[2] = vpn & 0x1ff;
}
...
PageTableEntry *findEntry(Mapping self, usize vpn)
{
    ...
    usize vpn_levels[3];
    getVpnLevels(vpn, vpn_levels);
    PageTableEntry *entry = &(rootTable->entries[vpn_levels[0]]);
    int i;
    for (i = 1; i <= 2; i++) {
        /* 页表不存在，创建新页表 */
        if (((*entry) & VALID )== 0) {
        ...
        entry = &(((PageTable *)accessVaViaPa(nextPageAddr))->entries[vpn_levels[i]]);
    }
    return entry;
}
...
void mapExtInterruptArea(Mapping m)
{
    Segment s1 = {(usize)0x0C000000 + KERNEL_MAP_OFFSET, (usize)0x0C001000 + KERNEL_MAP_OFFSET,
